...
plot(aggregate(pred, 30))


trSurface <- gdistance::transition(aggregate(pred, 30), transitionFunction = mean, directions = 8)
...
cgd <- circle_gd(vcf, coords, lyr = aggregate(pred, 30), distmat = distmat, 
                 radius = quantile(distmat, 0.05, na.rm = TRUE))

geodistmat <- make_distmat(coords_longlat, lyr = aggregate(pred, 30), 
                           parallel = TRUE, ncores = 3)

cg <- circle_gd(vcf, coords, lyr = aggregate(pred, 30), distmat = geodistmat, 
                radius = quantile(geodistmat, 0.015, na.rm = TRUE))
cg <- terra::mask(cg, rast(aggregate(pred, 30)))

wg <- window_gd(vcf, coords_longlat, lyr = aggregate(pred, 30), wdim = 5, crop_edges = TRUE)

par(mfrow = c(2,2), oma = rep(0,4), mar = rep(2,4), pty = "s")
raster_plot_gd(wg, bkg = aggregate(pred, 10), range = c(0.028, 0.137), legend = FALSE, 
        main = "Rectangle")
raster_plot_gd(cg, bkg = aggregate(pred, 10), range = c(0.028, 0.137), legend = FALSE,
        main = "Circle")
raster_plot_gd(cgd, bkg = aggregate(pred, 10), range = c(0.028, 0.137), legend = FALSE,
        main = "Resistance")
