...
PNW <- states[which(states$NAME %in% c("California", "Oregon", "Washington", "Nevada", "Idaho")), "STUSPS"]
...
plot(PNW, col = mako(1, begin = 0.7), border = "white", add = TRUE, main = "")
```

```{r, fig.width = 3, fig.height = 5}
par(mar = rep(0,4))
plot(PNW, col = mako(1, begin = 0.7), border = "white", lwd = 2, main = "")
```
...
set.seed(22)
pg <- window_gd(vcf, coords, lyr, stat = "pi", wdim = w, fact = f, rarify = TRUE, rarify_n = 2, rarify_nit = 5, L = nrow(vcf))
...
pgn <- window_gd(vcf, coords, lyr, stat = "pi", wdim = w, fact = f, rarify = FALSE, min_n = 2, L = nrow(vcf))
...
for(n in c(2, 3, 4)){
  ...
  for(w in c(3, 5)){
    for(f in c(4, 3, 2)){
      plot_gd(stk[[i]], PNW, zlim = c(0.02, 0.11), legend = FALSE, breaks = 100)
...
plot_gd(stk[[1]], zlim = c(0.02, 0.11), legend = TRUE, breaks = 100)
```
