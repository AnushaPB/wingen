load_sim_data <- function(){
  # load genetic data
  vcf <- vcfR::read.vcfR(here::here("sims_nlm/data/mod-sim_params_it-0_t-500_spp-spp_0.vcf"))
  assign("vcf", vcf, envir = .GlobalEnv)

  # load coords
  geo <- read.csv(here::here("sims_nlm/data/mod-sim_params_it-0_t-500_spp-spp_0.csv"))
  coords <- geo[,c("idx", "x", "y")]
  coords$y <- -coords$y
  assign("coords", coords, envir = .GlobalEnv)

  # load rasters
  lyr <- read.csv(here::here("sims_nlm/data/nlm.csv"), header = FALSE)
