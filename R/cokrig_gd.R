...
#'
cokrig_gd <- function(formula, r, grd, index = 1, coords = NULL,
...
krig_gd_lyr <- function(r, grd = NULL, coords = NULL,
  ...
  # create df
  krig_df <- make_cokrig_df(r, grd, coords)
  ...
  krig_r <- cokrig(formula, krig_df, krig_grid,
    autoKrige_output = autoKrige_output,
    zero_correction = zero_correction
  )
...
#'
#' @noRd
cokrig <- function(formula, krig_df, krig_grid, autoKrige_output = FALSE, zero_correction = TRUE) {
  # autokrige
  krig_res <- automap::autoKrige(formula, krig_df, krig_grid)
...
#'
#' @noRd
make_cokrig_df <- function(r, grd, coords = NULL) {
  ...
  grd_df <- data.frame(raster::extract(grd, krig_df[,c("x", "y")]))
  colnames(grd_df) <- names(grd)
  krig_df <- data.frame(krig_df, grd_df)

  # Remove na values
  krig_df <- krig_df[complete.cases(krig_df), ]
