...
#'
#' Perform interpolation of a "windowed" SpatRaster (e.g., moving-window genetic diversity estimates) using a thin-plate spline via the fields package.
...
tps_gd <- function(r, grd = NULL, index = 1, coords = NULL,
  ...
  if (is.logical(lower_bound) && lower_bound) {
    mn <- min(pts_df$layer, na.rm = TRUE)
    grd_df$layer[grd_df$layer < mn] <- mn
  }
  if (is.logical(upper_bound) && upper_bound) {
