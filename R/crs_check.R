...
crs_check_window <- function(lyr, coords) {
  ...
  if (is.na(coords_crs)) warning("No CRS found for the provided coordinates. Make sure the coordinates and the raster have the same projection. Assuming a euclidean system.")

  if (is.na(lyr_crs)) warning("No CRS found for the provided raster. Make sure the coordinates and the raster have the same projection. Assuming a euclidean system.")
...
crs_check_krig <- function(r, grd = NULL, coords = NULL) {
  ...
  if (!is.null(coords)) {
    ...
    if (is.na(coords_crs)) warning("No CRS found for the provided coordinates. Make sure the coordinates and the rasters have the same projection. Assuming euclidean coordinates.")
