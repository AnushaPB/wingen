...
```{r moving window, fig.width = 5, fig.height = 5, cache = TRUE, warning = TRUE}
...
`krig_method = "ordinary"` for ordinary/simple kriging (model formula:
 ...
 universal kriging (model formula: \~ x + y), which allows the mean 
 to vary across locations, while variance is held constant.
...
-   If `stat = pi` or `biallelic_richness`, `x` must be a dosage matrix
    with values of 0, 1, or 2 (*note:* rows must be individuals).

-   If `stat = Ho`, `x` must be a heterozygosity matrix where values of
    0 = homozygosity and values of 1 = heterozygosity (*note:* rows must be individuals).
