...
4.  `stat` - The genetic diversity summary statistic to calculate. 
    ...
    ene diversities overall (Ht), and Fis following Nei (1987).
    Population-based statistics (e.g., FST) normally reported by 
    `hierfstat::basic.stats()` are not included as they are not meaningful 
    within the individual-based moving windows.
...
window functions calculations with parallelization by using furrr. 
Checkout the [furrr package documentation](https://furrr.futureverse.org/) 
for more details on using parallelization.
...
*Note:* the parallel and ncores arguments have been deprecated as of wingen 2.1.0
