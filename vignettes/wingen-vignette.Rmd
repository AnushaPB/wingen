...
1. `vcf` - An object of type vcf with genotype data. The order of this file matters! The coordinate and genetic data should be in the same order, as there are currently no checks for this.
...
kgd_universal <- krig_gd(wgd, index = 1:2, lotr_lyr, disagg_grd = 2, krig_method = "ordinary")

plot_gd(kgd_universal, main = "Kriged pi")
plot_count(kgd_universal, main = "Kriged sample counts")
```
...
*Note:* kriging can produce values that fall outside the range of possible values (e.g. genetic diversity values less than 0 or greater than the possible maximum). By default, `krig_gd` converts all values greater than the maximum of the input raster, to that maximum (`upper_bound = TRUE`) and all values less than the minimum of the input raster, to that minimum (`lower_bound = TRUE`) to ensure that the values are bounded within the range of possible values. Users can turn off this functionality by setting `lower_bound` and `upper_bound` to `FALSE`. Users can also set there own custom `lower_bound` and `upper_bound` values (e.g. for heterozygosity you may want to set `lower_bound = 0` and `upper_bound = 1`).
