...
to `rarify_n` if rarefaction is used). We specify `method = window` for
...
1.  `maxdist` - maximum geographic distance used to define the neighborhood;
    ...
    either (1) a single numeric value or (2) a SpatRaster where each pixel is
...
settings. This time we specify `method = "circle"` and provide `maxdist`.

First, we calculate the distance matrix using `get_geodist()` which we can use
in both `preview_gd()` and` circle_gd()` to save time. If `distmat` is not
provided, it will be automatically calculated instead.

`get_geodist()` produces a distance matrix where the rows represent the cells
of the raster and the columns are the individuals. The values are the geographic
distance between each cell and each individual. Note that the input layer for
`get_geodist()` must have the same resolution as the input layer for
`preview_gd()` and` circle_gd()`. For example, if you want to run `circle_gd()`
with `fact = 3` and `lyr = lotr_lyr`, the input layer to `get_geodist()` must be
`aggregate(lotr_lyr, 3)`. Alternatively, you can first create a new aggregated
raster which you use for all functions, as we have done below.
...
```{r circle, fig.width = 5, fig.height = 5, cache = TRUE, warning = FALSE, eval = FALSE}
...
You can also perform `circle_gd()` (and `resist_gd()`) with a raster that sets the maximum distance (i.e., the radius) for each cell. In this case, we use the carrying capacity/conductance map and multiply it by 100 so that the radius will range from 0 to 100 based on the value of the raster. 

```{r variable circle, fig.width = 5, fig.height = 5, cache = TRUE, warning = FALSE, eval = FALSE}
...
1.  `maxdist` - maximum cost distance used to define the neighborhood; any
    ...
    of `maxdist` to determine the sensitivity of their results to this parameter.
...
`resist_gd()` takes a longer time to run and we recommend taking advantage of
parallelization, in particular for the calculation of the distance matrix with
`get_resdist()`. We also recommend first calculating the distance matrix with
`get_resdist()` and then providing it to `resist_gd()` using the `distmat`
argument as this allows for `resist_gd()` to be run quickly multiple times
(i.e., to test different parameters) by avoiding repeating the costly distance
calculations. Note that `con_lyr` and `lyr` must have the same spatial scale
(e.g. same resolution and extent).

As for `get_geodist()`, `get_resdist()` produces a distance matrix where the rows
represent the cells of the raster and the columns are the individuals. The
values are the resistance distance between each cell and each individual.

Again, we can use the `preview_gd()` function to preview parameter settings.
Here, we specify `method = "resist"` and provide `maxdist`.
We don't evaluate these chunks because it would take a while and should be run
in parallel, but we provide the code and resulting plot.

```{r, fig.width = 5, fig.height = 5, cache = TRUE, warning = FALSE, eval = FALSE}
distmat <- get_resdist(coords = lotr_coords, con_lyr = aggregate(lotr_lyr, 5), parallel = TRUE, ncores = 3)
...
`krig_method = "ordinary"` for ordinary/simple kriging (model formula: \~ 1) which assumes that the mean and variance are constant across space. Alternatively, `krig_method` can be set to `"universal"` for universal kriging (model formula: \~ x + y), which allows the mean to vary across locations, while variance is held constant.
...
-   `"hwe"` for the proportion of sites that are not in Hardy--Weinberg
    ...
    (other alpha levels can be specified by adding the sig argument; e.g.,
    `sig = 0.10`)
...
You can calculate multiple statistics as once by providing a vector of statstics names to `stat`.
