...
- return stop if circle_gd/resist_gd doesn't have enough samples (based on preview_gd) 
...
- make a circle_general() and a resist_general()
...
1.  `maxdist` - maximum geographic distance used to define neighborhood;
    ...
    can be thought of as the neighborhood radius) Similarly to `wdim` We
    encourage users to experiment with different values of `maxist` to
    determine the sensitivity of their results to this parameter and
    ideally set it with some knowledge of the study system in mind
    (e.g., the dispersal patterns and/or neighborhood size of the study
    organism).
...
1.  `maxdist` - maximum cost distance used to define neighborhood; any
    ...
    distance). As stated before, we encourage users to experiment with
    different values of `maxist` to determine the sensitivity of their
