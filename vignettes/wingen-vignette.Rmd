...
don't evaluate the resistance distance code because it would take a while 
and should be run in parallel, but we provide the created distance matrix
and results.

```{r, cache = TRUE, warning = FALSE, eval = FALSE, message = FALSE}
...
# code passable areas as 1 and then create an impassable area down the middle coded as NA
resist_lyr[] <- 1
...
# example resistance distance from coordinate 1
example <- resist_lyr
example[] <- distmat[, 1]
plot_gd(example, col = rocket(100), bkg = resist_lyr, main = "example resistance layer")
points(lotr_coords[1, ], col = "blue", pch = 3, cex = 2)
