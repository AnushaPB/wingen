...
gendist <- as.matrix(dist(dos, diag = FALSE, upper = FALSE))
geodist <- as.matrix(dist(lotr_coords, diag = FALSE, upper = FALSE))
plot(geodist, gendist)
```
```{r}
library(sp)
library(automap)
...
```{r}
dos <- vcf_to_dosage(lotr_vcf)
colnames(dos) <- paste0("X", colnames(dos))
df <- data.frame(lotr_coords, dos)
coordinates(df) = ~ x+y

f <- as.formula(paste(paste0(colnames(dos), collapse = "+"), "~ 1"))
variogram = automap::autofitVariogram(f, df)
plot(variogram)

df$env <- extract(lotr_lyr, lotr_coords)
f <- as.formula(paste(paste0(colnames(dos), collapse = "+"), "+ env ~ 1"))
variogram = automap::autofitVariogram(f, df)
plot(variogram)
variogram$var_model[2, "range"]
```
...
```
```{r}
distmat <- get_geodist(lotr_coords, aggregate(lotr_lyr, 5))
cgd17 <- circle_gd(lotr_vcf[1:100,],
  lotr_coords,
  lotr_lyr,
  stat = "hwe",
  maxdist = 17,
...
```{r}
vdf <- v$variogram
v$variogram$var_model
plot(vdf$exp_var$dist, vdf$exp_var$gamma)
```
