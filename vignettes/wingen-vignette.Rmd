...
2.  `winkrige_gd()` to use kriging to interpolate the moving window maps
...
`model_output = TRUE` in which case `winkrige_gd()` returns a list where
...
**Legacy method: krig_gd()** Users should note that `winkrige_gd()`
...
automatically generated variogram. We have found that `winkrige_gd()`
