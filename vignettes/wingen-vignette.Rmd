...
*Note:* Coordinates and rasters used in wingen should be in a Euclidean coordinate system (i.e., UTM coordinates) such that raster cell width and height are equal distances. Therefore, longitude-latitude systems should be transformed. An example of how this can be accomplished is shown here:
...
# Next, the coordinates and raster can then be projected to UTM like this:
coords_utm <- st_transform(coords_longlat, crs = "+proj=utm")
wgd_utm <- project(wgd_longlat, "+proj=utm")
# They can be switched back to latitude-longitude the same way (just replace "utm" with "longlat")
```
