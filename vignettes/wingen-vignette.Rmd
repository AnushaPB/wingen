...
to `rarify_n` if rarefaction is used). We specify `method = "window"`
for a preview of `window_gd()`; the other methods (i.e., `"circle"` and
`"resist"`) are discussed later.
...
don't evaluate these chunks because it would take a while and should be
run in parallel, but we provide the code and resulting plot below.
...
-   `"hwe"` for the proportion of sites that are not in Hardy--Weinberg
    ...
    (other alpha levels can be specified by adding the sig argument;
    e.g., `sig = 0.10`)
...
-   `"basic_stats"` for a series of statistics produced by hierfstat
    ...
    mean gene diversities within a population (Hs), overall gene
    diversities (Ht), corrected Htp, Dst, Dstp, Fst, Fstp, and Fis
    following Nei (1987)

You can also calculate multiple statistics at once by providing a vector
...
-   If `stat = "pi"` or `"biallelic_richness"`, `x` must be a dosage
    matrix with values of 0, 1, or 2.

-   If `stat = "Ho"`, `x` must be a heterozygosity matrix where values
    of 0 = homozygosity and values of 1 = heterozygosity.
