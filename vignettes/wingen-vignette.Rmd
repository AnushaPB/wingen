...
---
TODO: 
- crop edges update for resist_gd() and circle_gd()? not really meaningful for resist_gd()
- return stop if circle_gd doesn't have eonugh samples (nbased on preview_gd)
- preview_gd() update for circle_gd() and resist_gd()?
- fix error if min_n is set too high to make it more informative
- Think through min_n behavior (right now rarify_n defaults to min_n, and min_n defaults to 2, but what if a user sets rarify_n = 1? then they also need to set min_n = 1 for it to properly work...which is why originally I had min_n default to rarify_n if rarify_n was provided...ugh need to discuss with EAC and IJW). Should min_n default be 1? or maybe you should get a warning if min_n is greater than rarify_n? - i think the warning is a good idea beacuse i think min_n should default to 2
- replace ... with sig?
- check SpatRaster conversion for window_gd() in main...
- make resist gd work in non-parallel
- replace con_lyr with just lyr (why would you provide to diff things, they have to be the same spatial extent)
- seperate out sample count (add check so if all NA counts it doesn't run)
...
1. `window_gd()` uses a simple rectangular window with user-specified dimensions (the original method described in Bishop et al.)

2. `circle_gd()` uses a circular window with a user-specified radius

3. `resist_gd()` uses a resistance distance-based window with a user-specified max distance (this can be thought of as a circular window with a varying radius depending on the resistance of the landscape around it)
...
coordinate system such that raster cells are of equal sizes. Therefore, spherical systems
(including latitute-longitude coordinate systems) should be projected prior to use. 
An example of how this can be accomplished is shown below. If no CRS is provided, 
a warning will be given and wingen will assume the data are provided in a projected system. 
...
To create a moving window map with a rectangular window use `window_gd()`. 
The main additional arguments to `window_gd()` are:
...
1.  `maxdist` - maximum geographic distance used to define neighborhood; 
    any samples further than this distance will not be included (this can
    be thought of as the neighborhood radius) Similarly to `wdim` We encourage 
    users to experiment with different values of `maxist` to determine the 
    sensitivity of their results to this parameter and ideally set it with 
    some knowledge of the study system in mind (e.g., the dispersal patterns 
    and/or neighborhood size of the study organism). 
...
To create a moving window map with a resistance distances use `resist_gd()`. 
The main additional arguments to `resist_gd()` are:

1.  `maxdist` - maximum cost distance used to define neighborhood; any samples 
    further than this cost distance will not be included (this can be thought of as 
    the neighborhood radius, but in terms of cost distance). As stated before, we encourage 
    users to experiment with different values of `maxist` to determine the 
    sensitivity of their results to this parameter. 
    
2.  `con_lyr` - conductivity layer used to calculate resistance distances
    (higher values should mean greater conductivity). If one is not provided, the raster
    provided for `lyr` will be assumed to be the conductivity layer.

2.  `distmat` - optional distance matrix output from `get_resdist()`

`resist_gd()` takes a longer time to wrong and we recommend taking advantage of parallelization, in particular for the calculation of the distance matrix with `get_resdist()`. We also recommend first calculating the distance matrix with `get_resdist()` and then providing it to `resist_gd()` as this allows for `resist_gd()` to be run quickly multiple times (i.e., to test different parameters) by avoiding repeating the costly distance calculations. Note that `con_lyr` and `lyr` must have the same spatial scale (e.g. same resolution and extent).

We don't evaluate this chunk because it would take a while, but we provide the code and resulting plot. 

```{r moving window, fig.width = 5, fig.height = 5, cache = TRUE, warning = FALSE, eval = FALSE}
...
moving window raster produced by the moving window functions. This function uses the
...
Note that the moving window raster stack, including both the
...
the indices of the layers we want to krige, and the
...
To increase computational speed, users can perform the any of the moving window functions
...
- `"hwe"` for the proportion of sites that are not in Hardy–Weinberg equilibrium, calculated using pegas `hw.test()` at the 0.05 level (other alpha levels can be specified using the sig argument; e.g., `sig = 0.10`)
...
```
`"hwe"` and "`basic_stats`" take a bit longer to calculate, so here we change some parameters and reduce dataset for the purpose of the example: 
