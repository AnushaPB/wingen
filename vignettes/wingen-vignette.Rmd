...
bin_lyr <- aggregate(lotr_lyr, 20)
bin_lyr[] <- 1
bin_lyr[extent(40, 60, -100, 0)] <- NA
plot(bin_lyr)

distmat_bin <- get_resdist(coords = lotr_coords[1:2,], lyr = bin_lyr, parallel = TRUE, ncores = 3)

test <- bin_lyr
test[] <- distmat_bin[,1]
plot(test)
points(lotr_coords[1,])

test <- bin_lyr
test[] <- distmat_bin[,2]
plot(test)
points(lotr_coords[2,])
