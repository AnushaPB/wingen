...
# The plot_count function plots the sample count layer
ggplot_count(cgd) +
  ggtitle("Circle moving window sample counts")
```
...
ggplot_count(rgd) +
  ggtitle("Resistance moving window sample counts")
```
...
```{r universal krige results, fig.width = 5.5, fig.height = 5, cache = TRUE, warning = FALSE}
kgd_ordinary <-
  krig_gd(
    wgd,
    index = 1,
...
ggplot_gd(kgd_ordinary) + ggtitle("Kriged pi")
```
...
```{r, fig.width = 5.5, fig.height = 5, cache = TRUE, warning = FALSE, message = FALSE}
