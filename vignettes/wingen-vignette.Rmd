...
<<<<<<< Updated upstream
TODO: 
- Think through min_n behavior (right now rarify_n defaults to min_n, and min_n
defaults to 2, but what if a user sets rarify_n = 1? then they also need
to set min_n = 1 for it to properly work...which is why originally I had
min_n default to rarify_n if rarify_n was provided...ugh need to discuss
with EAC and IJW). Should min_n default be 1? or maybe you should get a
warning if min_n is greater than rarify_n? 

- replace ... with sig? 
=======
TODO: - Think through min_n behavior (right now rarify_n defaults to
min_n, and min_n defaults to 2, but what if a user sets rarify_n = 1?
then they also need to set min_n = 1 for it to properly work...which is
why originally I had min_n default to rarify_n if rarify_n was
provided...ugh need to discuss with EAC and IJW). Should min_n default
be 1? or maybe you should get a warning if min_n is greater than
rarify_n?
...
(2023)](http://doi.org/10.1111/2041-210X.14090).
...
1.  `window_gd()` uses a simple rectangular window with user-specified
    dimensions (the original method described in Bishop et al.)
...
3.  `resist_gd()` uses a resistance distance-based window with a
    user-specified max distance (this can be thought of as a circular
    window with a varying radius depending on the resistance of the
    landscape around it)
...
<<<<<<< Updated upstream
    ...
    a raster value of NA will always be assigned, even if the number 
    of samples isn't less than `rarify_n` (i.e., if `min = 2` and `rarify_n = 1`
    and the sample size is 1 in the window, then an NA value will be assigned; 
    to change this behavior so the genetic diversity is calculated, set `min_n = 1`)
=======
    ...
    samples isn't less than `rarify_n` (i.e., if `min = 2` and
    `rarify_n = 1` and the sample size is 1 in the window, then an NA
    value will be assigned; to change this behavior so the genetic
...
<<<<<<< Updated upstream
    ...
    If `rarify = FALSE`, rarefaction is not performed
...
=======
    ...
    and (2) occur such that: (1) if the number of samples in the window
    is less than the `min_n` argument, genetic diversity is not
    calculated and a raster value of `NA` is assigned, and (2) if the
...
Therefore, spherical systems (including latitute-longitude coordinate
...
to `rarify_n` if rarefaction is used). We specofy `method = window` for
a preview of `window_gd()`, we will diiscuss other methods (i.e.,
...
```{r window, fig.width = 5, fig.height = 5, cache = TRUE, warning = FALSE}
...
plot_count(wgd, bkg = lotr_range, main = "Moving window sample counts")
```

### Run moving window calculations with `circle_gd()`

To create a moving window map with a circular window use `circle_gd()`.
The main additional arguments to `circle_gd()` are:

1.  `maxdist` - maximum geographic distance used to define neighborhood;
    any samples further than this distance will not be included (this
<<<<<<< Updated upstream
    can be thought of as the neighborhood radius). Can be provided as either
    (1) a single numeric value or (2) a SpatRaster where each pixel is the 
    maximum distance to be used for that cell on the landscape (must be the 
    same spatial scale as `lyr`). Similarly to `wdim` We encourage users to 
    experiment with different values of `maxist` to determine the sensitivity 
    of their results to this parameter and ideally set it with some knowledge
    of the study system in mind (e.g., the dispersal patterns and/or neighborhood 
    size of the study organism). 

=======
    can be thought of as the neighborhood radius). Can be provided as
    either
    (1) a single numeric value or (2) a SpatRaster where each pixel is
        the maximum distance to be used for that cell on the landscape
        (must be the same spatial scale as `lyr`). Similarly to `wdim`
        We encourage users to experiment with different values of
        `maxist` to determine the sensitivity of their results to this
        parameter and ideally set it with some knowledge of the study
        system in mind (e.g., the dispersal patterns and/or neighborhood
        size of the study organism).
>>>>>>> Stashed changes
2.  `distmat` - optional distance matrix output from `get_geodist()`

Like before, we can use the `preview_gd()` function to preview parameter
settings. This time we specify `method = "circle"` and provide
`maxdist`.
...
```{r circle, fig.width = 5, fig.height = 5, cache = TRUE, warning = FALSE}
...
To create a moving window map with a resistance distances use
`resist_gd()`. The main additional arguments to `resist_gd()` are:

1.  `maxdist` - maximum cost distance used to define neighborhood; any
    samples further than this cost distance will not be included (this
    can be thought of as the neighborhood radius, but in terms of cost
    distance). Can be provided as either (1) a single numeric value or
<<<<<<< Updated upstream
    (2) a SpatRaster where each pixel is the maximum distance to be used 
    for that cell on the landscape (must be the same spatial scale as `lyr`).
    As stated before, we encourage users to experiment with different values 
    of `maxist` to determine the sensitivity of their results to this parameter.
=======

    (2) a SpatRaster where each pixel is the maximum distance to be used
        for that cell on the landscape (must be the same spatial scale
        as `lyr`). As stated before, we encourage users to experiment
        with different values of `maxist` to determine the sensitivity
        of their results to this parameter.
>>>>>>> Stashed changes
...
3.  `distmat` - optional distance matrix output from `get_resdist()`

`resist_gd()` takes a longer time to wrong and we recommend taking
advantage of parallelization, in particular for the calculation of the
distance matrix with `get_resdist()`. We also recommend first
calculating the distance matrix with `get_resdist()` and then providing
it to `resist_gd()` as this allows for `resist_gd()` to be run quickly
multiple times (i.e., to test different parameters) by avoiding
repeating the costly distance calculations. Note that `con_lyr` and
`lyr` must have the same spatial scale (e.g. same resolution and
extent).

We don't evaluate these chunks because it would take a while and should
be run in parallele, but we provide the code and resulting plot.

Again, we can use the `preview_gd()` function to preview parameter
settings. Here, we specify `method = "resist"` and provide `maxdist`. We
also generate a distance matrix separately to save time since this can
be used both in `preview_gd()` and `resist_gd()`.

```{r, fig.width = 5, fig.height = 5, cache = TRUE, warning = FALSE, eval = FALSE}
distmat <- get_resdist(coords = lotr_coords, con_lyr = aggregate(lotr_lyr, 5), ncores = 3)
...
![output from preview_gd()](resist_plots/resist_preview.png)

```{r resist, fig.width = 5, fig.height = 5, cache = TRUE, warning = FALSE, eval = FALSE}
...
![genetic diversity output from resist_gd()](resist_plots/resist_pi.png)
<<<<<<< Updated upstream
![sample count output from resist_gd()](resist_plots/resist_samplecount.png)
=======
![sample count output from
resist_gd()](resist_plots/resist_samplecount.png)
>>>>>>> Stashed changes
...
-   `"pi"` for nucleotide diversity (default) calculated using hierfstat
    `pi.dosage()`
...
-   `"biallelic_richness"` for average allelic richness across all sites
    for a biallelic dataset (this option is faster than
    `"allelic_richness"`)

-   `"hwe"` for the proportion of sites that are not in Hardy--Weinberg
    equilibrium, calculated using pegas `hw.test()` at the 0.05 level
    (other alpha levels can be specified using the sig argument; e.g.,
    `sig = 0.10`)

-   `"basic_stats"` for a series of statistics produced by hierfstat
    `basic.stats()` including mean observed heterozygosity (same as Ho),
    mean gene diversities within population (Hs), Gene diversities
    overall (Ht), corrected Htp, Dst, Dstp, Fst, Fstp, and Fis following
    Nei (1987)
...
par(pty = "s")
plot_gd(multistat_wgd, index = 1:3, bkg = lotr_range)
```

`"hwe"` and "`basic_stats`" take a bit longer to calculate, so here we
change some parameters and reduce dataset for the purpose of the
example:
...
```{r, fig.width = 5, fig.height = 5, warning = FALSE}
par(pty = "s")
plot_gd(bs_wgd, index = 1:10, bkg = lotr_range)
```
...
-   If `stat = het`, `x` must be a heterozygosity matrix where values of
    0 = homozygosity and values of 1 = heterozygosity.
...
- add diff distance calculations into new make_distmat function
=======
...
>>>>>>> Stashed changes
```{r}
wgd <- window_gd(lotr_vcf, lotr_coords, lotr_lyr, wdim = 7, fact = 3, rarify = TRUE)
plot_gd(wgd)

wmean1 <- wmean_gd(wgd, maxdist = 10)
plot_gd(wmean1)

wmean2 <- purrr::map(seq(2, 8, 2), ~wmean_gd(wgd, beta = .x))
purrr::walk(wmean2, plot_gd)
...
# ADD MAXDIST VALUE
wmean3 <- wmean_gd(lyr1, maxdist = 20)
plot_gd(lyr1)
plot_gd(wmean3)
```
