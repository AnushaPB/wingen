...
To demonstrate how wingen works, we will use a subset the data from the Bishop et al. simulation example (in review). These simulations were created using Geonomics (Terasaki Hart et al., 2022) to generate a realistic landscape genomic dataset. In this simulation, spatial variation in genetic diversity is produced by varying population size and gene flow across the landscape via heterogenous carrying capacity and conductance surfaces. These surfaces are based on an example digital elevation model of Tolkien’s Middle Earth produced by the Center for Geospatial Analysis at William & Mary ([Robert, 2020](https://scholarworks.wm.edu/asoer/3/)).
...
1. `gen` - An object of type vcf or a path to a vcf file with genotype data. The order of this file matters! The coordinate and genetic data should be in the same order, as there are currently no checks for this.
...
*Note:* Coordinates and rasters used in wingen should be in a Euclidean coordinate system (i.e., UTM coordinates) such that raster cell width and height are equal distances. Therefore, longitude-latitude systems should be transformed. An example of how this can be accomplished is shown below. If no CRS is provided, a warning will be given and wingen will assume a euclidean system.
...
**Method 2:** mask the layer using a species range map (in this case, a sf polygon) to exclude areas falling outside the species range.
...
**Method 3:** mask the layer using a spatial Kernel Density Estimation (KDE) of sample density to exclude areas with low sampling density:
...
# Converting from genind to VCF

Some users may have files in a genind format instead of vcf. The dartR package can be used to convert from genind to vcf. Currently, dartR requires SNPRelate which must be installed from BiocManager. Below we provide code to install the required packages and convert from genind to vcf:
```{r}
if (!require("BiocManager", quietly = TRUE)) install.packages("BiocManager")
if (!require("SeqArray", quietly = TRUE)) BiocManager::install("SeqArray")
if (!require("devtools", quietly = TRUE)) install.packages("devtools")
if (!require("radiator", quietly = TRUE)) devtools::install_github("thierrygosselin/radiator")
library(radiator)

df <- genind2df(genind, oneColPerAll = TRUE)
vcf <- dartR::gl2vcf(genlight)

## Not run:
require("dartR.data")
dartR::gl2vcf(platypus.gl,
...
devtools::install_github("thierrygosselin/radiator", INSTALL_opts = c("--no-multiarch"))

library(radiator)
genind <- vcfR2genind(mini_vcf)
genind@pop <- rep(factor(1), nrow(genind@tab))
genomic_converter(data = genind, output = "vcf")
```
