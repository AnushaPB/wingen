...
system.time(gdmap <- window_gd(ex_vcf, ex_coords, ex_lyr, stat = "pi", fact = 20, wdim = 5, rarify = FALSE))
...
krig_gdmapr <- krig_gd(gdmapr, disagg = 10)
plot(krig_gdmapr, col = viridis::magma(100), box = FALSE, axes = FALSE)

krig_gdmapr <- krig_gd(gdmapr, ex_lyr, agg = 5)
plot(krig_gdmapr, col = viridis::magma(100), box = FALSE, axes = FALSE)
```
```{r, fig.width = 5, fig.height=5}
krig_msk <- mask_gd(gdmapr, 4)
plot(krig_msk, col = viridis::magma(100), box = FALSE, axes = FALSE)

krig_msk <- mask_gd(krig_gdmapr, 4)
plot(krig_msk, col =  viridis::magma(100),  box = FALSE, axes = FALSE)
```

```{r, fig.width = 5, fig.height = 5}
krig_msk <- mask_gd(krig_gdmapr, 4, plot = TRUE, bkg.col = "black")
```
...
gd_krig <- krig_gd(gd_stack, ex_lyr, agg = 5)

par(mfrow = c(1,3))
gd_mask <- mask_gd(gd_krig, 4, plot = TRUE, bkg.col = "black")
```
