...
- replace ... with sig? 
- make sure preview_gd won't cause any problems? deprecation?
...
it to `resist_gd()` using the `distmat` argument as this allows for `resist_gd()` to 
be run quickly multiple times (i.e., to test different parameters) by avoiding
...
be run in parallel, but we provide the code and resulting plot.
