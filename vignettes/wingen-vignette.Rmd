...
---
TODO: 
- crop edges update for resist_gd() and circle_gd()?
- preview_gd() update for circle_gd()
- fix error if min_n is set too high to make it more informative
- Think through min_n behavior (right now rarify_n defaults to min_n, and min_n defaults to 2, but what if a user sets rarify_n = 1? then they also need to set min_n = 1 for it to work...which is why originally I had min_n default to rarify_n if rarify_n was provided...ugh need to discuss with EAC and IJW). SHould min_n default be 1? or maybe you should get a warning if min_n is greater than rarify_n?
- replace ... with sig?
