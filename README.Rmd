...
Install the released version of wingen from CRAN (⚠️ temporarily unavailable, see notice above):
``` r
...
Next, the output from `window_gd()` can be interpolated using kriging with the `winkrige_gd()` function.
...
Finally, the output from `winkrige_gd()` (or `window_gd()`) can be masked to exclude areas that fall outside of the study area or that were undersampled.
