...
# system time sims
```{r}
vcf <- read.vcfR("data/mod-sim_params_it-0_t-100_spp-spp_0.vcf")
geo <- read.csv("data/mod-sim_params_it-0_t-100_spp-spp_0.csv")
coords <- geo[,c("idx","x","y")]



set.seed(42)
if(!file.exists("samples_seed42.csv")){
...
preview_gd(lyr, subcoords, wdim = 5, fact = 3, sample_count = FALSE)
  ...
  for(m in measures){
    ...
    for(rarify in c("TRUE", "FALSE")){
      for(d in datasets){
        ...
        if(m == "heterozygosity"){zlim = c(0.05,0.40)}
