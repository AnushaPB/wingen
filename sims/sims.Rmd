...
tr_k <- krig_gd(tr[["res"]], lyr, disagg = 2, resample = TRUE)
...
tr_k_2 <- krig_gd(tr[["res"]], lyr, disagg = 2, resample = TRUE)
for(i in c(1, 2, 4, 8)){
...
tr_k_2F <- krig_gd(tr[["res"]], lyr, disagg = 2, resample = FALSE)
for(i in c(1, 2, 4, 8)){
...
tr_k_2F <- krig_gd(tr[["res"]], lyr, disagg = 1, resample = FALSE)
for(i in c(1, 2, 4, 8)){
  tr_k_mask <- mask_gd(tr_k_2F, i)
  plot(lyr, col = magma(1, begin = 0.1, alpha = 0.1), axes = FALSE, box = FALSE, zlim = c(0.01,1), legend = FALSE)
  plot(tr_k_mask, col = viridis::magma(100), axes = FALSE, box = TRUE, legend = FALSE, add = TRUE, zlim = c(0.18, 0.47))
...
tr_k_2F <- krig_gd(tr[["res"]], lyr, disagg = 1, resample = TRUE)
for(i in c(1, 2, 4, 8)){
  tr_k_mask <- mask_gd(tr_k_2F, i)
  plot(lyr, col = magma(1, begin = 0.1, alpha = 0.1), axes = FALSE, box = FALSE, zlim = c(0.01,1), legend = FALSE)
  plot(tr_k_mask, col = viridis::magma(100), axes = FALSE, box = TRUE, legend = FALSE, add = TRUE, zlim = c(0.18, 0.47))
...
msk_lyr <- get_divout(file.name = "rr", rarify = TRUE, measure = "pi", nsamp = 100)
...
for(m in measures){
  ...
  for(d in datasets){ 
    for(rarify in c("TRUE", "FALSE")){
      r <- get_divout(file.name = d, rarify = rarify, measure = m, nsamp = 100)
      r <- mask(r, msk_lyr)
