...
# system time sims
```{r}
vcf <- read.vcfR("data/mod-sim_params_it-0_t-10000_spp-spp_0.vcf")
geo <- read.csv("data/mod-sim_params_it-0_t-10000_spp-spp_0.csv")
  ...
  for(m in measures){
    ...
    for(rarify in c("TRUE", "FALSE")){
      for(d in datasets){
        ...
        if(is.null(r)) next
        r <- mask(r, msk_lyr)
        if(m == "pi"){zlim = c(0,0.3)}
        if(m == "biallelic_richness"){zlim = c(1.1,1.7)}
        if(m == "heterozygosity"){zlim = c(0,0.2)}
