...
vcf <- read.vcfR("data/mod-sim_params_it-0_t-1000_spp-spp_0.vcf")
...
geo <- read.csv("data/mod-sim_params_it-0_t-1000_spp-spp_0.csv")
...
if(TRUE){
  ...
  write.csv( data.frame(inds = si), here("sims", "data", "samples_seed42.csv"), row.names = FALSE)
} else {
  message("loading existing file")
  df <- read.csv( here("sims", "data", "samples_seed42.csv"))
  si <- df$inds
}
...
tr <- sim(subvcf, subcoords, lyr, stat = "pi", wdim = 3, fact = 3, rarify_n = 2, rarify_nit = 5, rarify = FALSE, parallel = TRUE, nloci = nrow(subvcf@gt))
  ...
  for(m in measures){
    ...
    for(rarify in c("TRUE", "FALSE")){
      for(d in datasets){
        ...
        if(m == "pi") {zlim <- c(0, 0.3); r[r < 0] <- 0}
        if(m == "biallelic_richness") {zlim <- c(1, 1.7); r[r < 1] <- 1}
        if(m == "heterozygosity") {zlim <- c(-0.02,0.4)}
        r <- mask_gd(r, lyr, minval = 0.01)
        plot(r,  col = magma(10), box = TRUE, axes = FALSE, main = "", legend = FALSE)    
      }
    }
  }
}
...
ggplot(data = tdf, aes(x = factor(nsamp), y = time, fill = stat)) +
  ...
  geom_text(aes(factor(200), 60, label = "1 min", vjust = -1, hjust = -0.7), col = "darkgray", fontface = "italic") +
  geom_hline(yintercept = 60*5, linetype = "dashed", col = "gray", lwd = 1) + 
  geom_text(aes(factor(200), 60*5, label = "5 min", vjust = -1, hjust = -0.7), col = "gray", fontface = "italic") +
  geom_hline(yintercept = 60*10, linetype = "dashed", col = "gray", lwd = 1) + 
  geom_text(aes(factor(200), 60*10, label = "10 min", vjust = -1, hjust = -0.4), col = "lightgray", fontface = "italic") +
...
for(f in c(2, 3, 4)){
  for(w in c(3, 5, 7)){
    ...
    stk1[[i]] <-  tr_k_mask
    stk2[[i]] <-  res[[1]]
    writeRaster(stk1[[i]], here("sims","outputs", "wftest", paste0(f,w,".tif")))
    writeRaster(stk2[[i]], here("sims","outputs", "wftest", paste0(f,w,"krig.tif")))
...
for(f in c(4,3,2)){
  for(w in c(3,5,7)){
    r <- raster(here("sims","outputs", "wftest", paste0(f,w,".tif")))
    plot(lyr, col = magma(1, begin = 0.1, alpha = 0.1), axes = FALSE, box = FALSE, zlim = c(0.01,1), legend = FALSE)
    plot(r, col = magma(10), axes = FALSE, box = FALSE, legend = FALSE, zlim = c(-0.02, 0.41), add = TRUE)
...
for(f in c(4,3,2)){
  for(w in c(3,5,7)){
    r <- raster(here("sims","outputs", "wftest", paste0(f,w,"krig.tif")))
    plot(lyr, col = magma(1, begin = 0.1, alpha = 0.1), axes = FALSE, box = FALSE, zlim = c(0.01,1), legend = FALSE)
    plot(r, col = magma(10), axes = FALSE, box = FALSE, legend = FALSE, zlim = c(-0.02, 0.41), add = TRUE)
    rect(0, -105, 100, 0, border = "black")
    i <- i + 1
...
plot(stk2[[i-1]], col = magma(10), axes = FALSE, box = FALSE, zlim = c(-0.02, 0.41))
```
...
for (m in c("pi", "biallelic.richness", "het")){
  for (n in c(1000, 5000, 10000)){
   tr_k <- raster(here("sims", "outputs", paste0(m,"_",n,"krig.tif")))
...
for(n in c(1000,5000,10000)){
  kde <- raster(here("sims", "outputs", paste0(n,"_kde.tif")))
