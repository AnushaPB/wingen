...
res1 <- window_gd(subvcf, subcoords, lyr, stat = "het", fact = 3, wdim = 5, rarify = TRUE, rarify_n = 6, rarify_nit = 10, parallel = TRUE)
res2 <- window_gd(subvcf, subcoords, lyr, stat = "het", fact = 3, wdim = 5, rarify = FALSE, min_n = 4, parallel = TRUE)
stopCluster(cl)
plot(res1[[1]], col = magma(100))
points(subcoords[,c("x","y")])
plot(res2[[1]], col = magma(100))
points(subcoords[,c("x","y")])
```
...
tr <- sim(subvcf, subcoords, lyr, stat = "pi",fact = 3, wdim = 5, min_n = 4, rarify_n = 4, rarify_nit = 5, rarify = FALSE, parallel = TRUE)
...
th <- sim(subvcf, subcoords, lyr, stat = "pi", fact = 3, wdim = 5, rarify = TRUE, rarify_n = 6, rarify_nit = 10, parallel = TRUE)
...
for(f in c(3, 5, 7)){
  for(w in c(3, 5, 7)){
    res <- window_gd(subvcf, subcoords, lyr, stat = "pi", fact = f, wdim = w, rarify = TRUE, rarify_n = 6, rarify_nit = 10, parallel = TRUE)
    stk <- stack(stk, res[[1]])
...
resw <- purrr::map(c(3, 5, 7, 9, 11, 13), time_test, "wdim", subvcf, subcoords, lyr, fact = 2, stat = "pi", rarify = TRUE, rarify_n = 10, rarify_nit = 10)
...
m <- matrix(1, ncol = 14, nrow = 14)
r <- raster(m)
pts <- rasterToPoints(r)
dcoords <- data.frame(pts[,c("x","y")])
dvcf <- vcf[1:10000,c(1, sample(nrow(dcoords)) + 1)]

dr <- raster(matrix(1,ncol = 100, nrow = 100))

par(pty ="s")
plot(dr)
points(dcoords)

resfr <- purrr::map(c(2, 5, 10, 20), time_test, "fact", dvcf, dcoords, dr, stat = "pi", wdim = 5, rarify = FALSE, min_n = 2)

plot_time_test(resfr, stat = "fact")
```
...
```
```{r}
resf <- purrr::map(c(2, 5, 10, 20), time_test, "fact", dvcf, dcoords, dr, stat = "pi", wdim = 5, rarify = TRUE, rarify_n = 2, rarify_nit = 10, min_n = 2)
...
set.seed(42)
ress <- purrr::map(c("het", "pi"), time_test, "stat", subvcf, subcoords, lyr, fact = 3, wdim = 5, min_n = 4, rarify_n = 4, rarify_nit = 5, nloci = 100000)
...
# object has tr with first slot as time and second as AR rasters
# created using this script: allelic_richness_time_test.R
load("ar_results.RData")
plot(lyr, col = magma(1, begin = 0.1, alpha = 0.1), axes = FALSE, box = FALSE, zlim = c(0.01,1), legend = FALSE)
plot(tr[["res"]]$allelic_richness, col = viridis::magma(100, alpha = 1.0), main = "Allelic Richness", axes = FALSE, box = TRUE, legend = TRUE, add = TRUE, legend.width=1.5, legend.shrink=0.8)
```
